...
def state_extrapolation(current_state_vector, state_transition_matrix):
    ...
    :param current_state_vector: state vector (including all estimated states of the system)
                                     after the update step,
    :param state_transition_matrix: matrix for the implementation of the dynamic model.

    :returns: next_state_vector: state vector at the next time step, before the update.
    """
...
def state_update(previous_state_vector, measurements, observation_matrix, kg):
    ...
    :param previous_state_vector: state vector (including all estimated states of the system)
                            before the update step,
    :param measurements: vector storing all measured states at a specific time step,
    :param observation_matrix: matrix converting the measurement vector of the system into the
                         state vector,
    :param kg: Kalman gain (it weights the measurements).

    :returns: current_state_vector: state vector after the update.
    """
...
def kalman_gain(previous_state_cov_matrix, measurement_cov_matrix, observation_matrix):
    ...
    :param previous_state_cov_matrix: state covariance matrix (defining the uncertainty in each of the estimates)
                                      before the update step,
    :param measurement_cov_matrix: covariance matrix defining the uncertainty in each of the measurements,
    :param observation_matrix: matrix converting the measurement vector of the system into the
                               state vector.

    :returns: kg: Kalman gain (it weights the measurements).
    """
...
def covariance_extrapolation(current_state_cov_matrix, state_transition_matrix):
    ...
    :param current_state_cov_matrix: state covariance matrix (defining the uncertainty in each of the estimates)
                                       after the update step,
    :param state_transition_matrix: matrix for the implementation of the dynamic model.

    :returns: next_state_cov_matrix: state covariance matrix at the next time step, before the update.
    """
...
def covariance_update(previous_state_cov_matrix, measurement_cov_matrix, observation_matrix, kg):
    ...
    :returns: current_state_cov_matrix: state covariance matrix after the update step.
    """
